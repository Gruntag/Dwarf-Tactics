...
const FORGE_RESOURCE_KEY = "dragonFire" as const;
const forgeResource = RESOURCES[FORGE_RESOURCE_KEY];
const forgeIngots = ref(getResourceCount(FORGE_RESOURCE_KEY));
...
const overlaySubtitle = computed(() => {
  switch (phase.value) {
    case "victory":
      return "You bent the molten torrents back upon themselves. The forge glows brighter with your legend.";
    case "defeat":
...
const endGame = (result: "victory" | "defeat") => {
  ...
  cancelAnimation();
  if (result === "victory") {
    statusMessage.value = "The inferno yields to your mastery.";
  } else {
...
watch(
  ...
  (next) => {
    if (next === "running") {
      forgeIngots.value = getResourceCount(FORGE_RESOURCE_KEY);
    }

    if (next === "victory" && !pointGranted) {
      forgeIngots.value = addResource(FORGE_RESOURCE_KEY);
      pointGranted = true;
      toast({
        title: `${forgeResource.plural} Secured`,
        description: `Flame bows to your command. ${forgeResource.singular} captured.`,
        variant: "success",
      });
    }
  }
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div class="flex items-center gap-2 rounded-lg border border-primary/30 bg-card/80 px-4 py-2 text-sm text-foreground backdrop-blur">
            ...
            <div class="flex flex-col leading-tight text-right">
              ...
              <span class="text-lg font-semibold text-foreground">{{ forgeIngots }}</span>
            </div>
          </div>
        </div>
